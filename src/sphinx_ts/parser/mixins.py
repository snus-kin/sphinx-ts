...
class NamedObjectMixin:
    ...
    This mixin eliminates the duplication of __lt__, __eq__, and __hash__ methods
    across all TypeScript AST node classes that have a 'name' attribute.
    """

    name: str  # Type hint for the name attribute that implementing classes must have
    ...
    def __eq__(self, other: object) -> bool:
        """Support equality comparison of named objects by name (case-insensitive)."""
        if isinstance(other, self.__class__):
            return self.name.lower() == other.name.lower()
        return NotImplemented
